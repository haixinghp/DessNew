...
static int8_t App_GUI_FingerCheck( uint16_t *pageId, uint8_t userLimit )
{
	...
	switch( FingerCheckFlow )
	{
		...
		case 2: //等待检测结果	
				...
				{
					if(SystemSeting.FingerOrVeinUnlockSuccessCnt > 65534)
					{
						SystemSeting.FingerOrVeinUnlockSuccessCnt = 0;
					}
					else
					{
						SystemSeting.FingerOrVeinUnlockSuccessCnt++;    
					}
					my_printf( "---------SystemSeting.FingerOrVeinUnlockSuccessCnt = %d \n" , SystemSeting.FingerOrVeinUnlockSuccessCnt);
					(void)SystemWriteSeting( (uint8_t *)&SystemSeting.FingerOrVeinUnlockSuccessCnt, sizeof SystemSeting.FingerOrVeinUnlockSuccessCnt );
				...
				{ 
                    ...
                    #endif
					{
						if(SystemSeting.FingerOrVeinUnlockFailCnt > 65534)
						{
							SystemSeting.FingerOrVeinUnlockFailCnt = 0;
						}
						else
						{
							SystemSeting.FingerOrVeinUnlockFailCnt++;    
						}    
						my_printf( "---------SystemSeting.FingerOrVeinUnlockFailCnt = %d \n" , SystemSeting.FingerOrVeinUnlockFailCnt);
						(void)SystemWriteSeting( (uint8_t *)&SystemSeting.FingerOrVeinUnlockFailCnt, sizeof SystemSeting.FingerOrVeinUnlockFailCnt );
						APP_FINGER_Sleep();  //关闭指纹模组
						FingerCheckFlow = 0;
						return -1;
					}
...
static int8_t App_GUI_FaceCheck( uint8_t userLimit, uint16_t *pageId, uint8_t *unlockStatus)
{
	...
	if( FACE_VERIFY_SUCCESS == faceret )           //验证成功
	{
		...
		if(SystemSeting.FaceOrIrisUnlockSuccessCnt > 65534)
		{
			SystemSeting.FaceOrIrisUnlockSuccessCnt = 0;
		}
		else
		{
			SystemSeting.FaceOrIrisUnlockSuccessCnt++;    
		} 
		my_printf( "---------SystemSeting.FaceOrIrisUnlockSuccessCnt = %d \n" , SystemSeting.FaceOrIrisUnlockSuccessCnt);
		(void)SystemWriteSeting( (uint8_t *)&SystemSeting.FaceOrIrisUnlockSuccessCnt, sizeof SystemSeting.FaceOrIrisUnlockSuccessCnt );
		return 1;
	}
		  ...
		  || FACE_VERIFY_EE_FAIL== faceret || FACE_VERIFY_ADMIN_FAIL== faceret )   //验证失败
	{
		...
		if(SystemSeting.FaceOrIrisUnlockFailCnt > 65534)
		{
			SystemSeting.FaceOrIrisUnlockFailCnt = 0;
		}
		else
		{
			SystemSeting.FaceOrIrisUnlockFailCnt++;    
		}
		my_printf( "---------SystemSeting.FaceOrIrisUnlockFailCnt = %d \n" , SystemSeting.FaceOrIrisUnlockFailCnt);
		(void)SystemWriteSeting( (uint8_t *)&SystemSeting.FaceOrIrisUnlockFailCnt, sizeof SystemSeting.FaceOrIrisUnlockFailCnt );
		return -1;
	}
		...
		switch( checkKeyType )
		{
			...
			case E_CHECK_FACE:      //人脸/虹膜验证
			{
				...
				if( faceret == 1 )        //验证成功
				{
					...
					checkKeyType = E_CHECK_DEFAULT;
					WifiLockMeg.UnlockWay1SuccessTime[0] = (uint8_t)((SystemSeting.FaceOrIrisUnlockSuccessCnt) >> 8);
					WifiLockMeg.UnlockWay1SuccessTime[1] = (uint8_t)((SystemSeting.FaceOrIrisUnlockSuccessCnt));
					WifiLockMeg.UnlockWay1FailTime[0] = (uint8_t)((SystemSeting.FaceOrIrisUnlockFailCnt) >> 8);
					WifiLockMeg.UnlockWay1FailTime[1] = (uint8_t)((SystemSeting.FaceOrIrisUnlockFailCnt));
					my_printf("-----------SystemSeting.FaceOrIrisUnlockFailCnt = %d\n", SystemSeting.FaceOrIrisUnlockFailCnt);
					if( FUNCTION_ENABLE == App_GUI_GetDoubleCheckSwSts() )  //双重认证开启
					{
						KeyTppe = App_GUI_GetDoubleCheckType(); 
...
//							App_WIFI_CommomTx( WIFI_CMD_UPLOAD_UNLOCK_MEG );
							UploadUnlockDoorMegEnable = 1;
							WifiLockMeg.UnlockWay2SuccessTime[0] = (uint8_t)((SystemSeting.FingerOrVeinUnlockSuccessCnt) >> 8);
							WifiLockMeg.UnlockWay2SuccessTime[1] = (uint8_t)((SystemSeting.FingerOrVeinUnlockSuccessCnt));
							WifiLockMeg.UnlockWay2FailTime[0] = (uint8_t)((SystemSeting.FingerOrVeinUnlockFailCnt) >> 8);
							WifiLockMeg.UnlockWay2FailTime[1] = (uint8_t)((SystemSeting.FingerOrVeinUnlockFailCnt));
...
//							App_WIFI_CommomTx( WIFI_CMD_UPLOAD_UNLOCK_MEG );
						...
						SystemEventLogSave( VEIN_OPEN, fingerPageId );
						#endif
						WifiLockMeg.UnlockWay1SuccessTime[0] = (uint8_t)((SystemSeting.FingerOrVeinUnlockSuccessCnt) >> 8);
						WifiLockMeg.UnlockWay1SuccessTime[1] = (uint8_t)((SystemSeting.FingerOrVeinUnlockSuccessCnt));
						WifiLockMeg.UnlockWay1FailTime[0] = (uint8_t)((SystemSeting.FingerOrVeinUnlockFailCnt) >> 8);
						WifiLockMeg.UnlockWay1FailTime[1] = (uint8_t)((SystemSeting.FingerOrVeinUnlockFailCnt));
						my_printf("------------SystemSeting.FingerOrVeinUnlockFailCnt = %d\n", SystemSeting.FingerOrVeinUnlockFailCnt);
						App_GUI_MenuJump( EM_MENU_OPEN_DOOR );
						return;
					}
...
static void App_GUI_GotoAppModelMenu( void )	  //APP设置模式
{
	...
	if( EM_MENU_STEP_1 == MenuItem.CurMenuNum )
	{
		...
		#if defined ST_CAM_FUNCTION_ON || defined OB_CAM_FUNCTION_ON  //二代猫眼CAMERA_IRQ拉低且当前无任务
		if( (0 == HAL_EXPORT_PinGet( EM_CAMERA_IRQ )) && (AppFaceWorkPro.TaskFlow == TASK_POWERDOWN))
		{
			my_printf( "------HAL_EXPORT_PinGet( EM_CAMERA_IRQ ) = 0\n" ); 
...
static void App_GUI_BellVideoMenu( void )		  //门铃视频处理菜单	
{
    if( DispFuncPtrPre != DispFuncPtr )
    {
		...
		APP_SCREEN_Operate(EM_SCREEN_APP_FLOW0_SHOW_MOTION , EM_SCREEN_FLOW0_ACT_GREET);
	}
	#if defined OB_CAM_FUNCTION_ON || defined ST_CAM_FUNCTION_ON //二代人脸格式
	/*---------------分离屏蓝牙任务调度-----------------*/	
	APP_BleServerProcess();
